				...
				{Array.from({ length: totalPages || 1 }, (_, i) => i + 1).map((page) => (
					<button
						...
						className={`pagination-button rounded-md p-[5px] px-[9px] ms-3 ${
							currentPage === page ? "bg-primaryBtn text-black" : "bg-white"
						}`}
	...
	return (
		<div className="container mx-auto px-4 bg-background">
			<header className="flex justify-between items-center py-5">
				<button type="button" className="bg-primaryBtn px-4 py-2 rounded-3xl" onClick={handleClickAdd}>
					Add New Post
				</button>
				<LoginRoute />
			</header>

			<div className="text-center mt-3 mb-4 text-4xl text-title">Post List</div>
